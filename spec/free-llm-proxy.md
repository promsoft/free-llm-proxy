...
- `stream=true` **поддерживается** — ответ `text/event-stream` (SSE) в
  формате OpenAI Chat Completions chunks, с `data: [DONE]\n\n` в конце.
- Логика обработки — см. §5.
- Ответ — как у OpenRouter (формат OpenAI), с дополнительным полем
  `x-free-llm-proxy-model: <chosen_model_id>` в response headers (и для
  non-stream, и для stream — заголовок выставляется до первого чанка).
...
| HTTP `401` или `403` от OpenRouter                | **не fallback** — это сигнал плохого `OPENROUTER_API_KEY`. Отвечаем клиенту `502 upstream_auth_error` с подсказкой и хвостом ключа (`...XXXX (len=N)`), чтобы было видно «не подставился ли placeholder». |
| HTTP `4xx`, кроме `429`/`401`/`403`               | **не fallback** — это ошибка запроса, отдать клиенту as-is |
...
### 5.6. Streaming: правила fallback в потоке
- Пока **первый SSE-чанк не отправлен клиенту**, поведение полностью
  совпадает с non-stream: ошибка `create()` → классификация → cooldown
  → следующая модель. На 4xx (≠429) — passthrough клиенту с правильным
  HTTP-статусом, fallback не триггерится. Если все попытки провалились
  — `503 all_models_unavailable`.
- **Как только первый чанк ушёл**, fallback невозможен (HTTP-статус и
  заголовки уже отправлены). Любая ошибка в середине потока:
  - кладёт модель в cooldown (как обычно — по правилам §5.3, §5.4);
  - в поток отдаётся **один** дополнительный кадр `data:
    {"error": {"message": "...", "code": "rate_limited|upstream_error",
    "type": "upstream_error"}}\n\n`, а затем `data: [DONE]\n\n` —
    клиент видит частичный успех + явный сигнал ошибки.
- Метрика `freellm_requests_total{status}` для стрима с mid-stream
  ошибкой выставляется со статусом `200/mid_error`, чтобы не путать с
  чисто успешными.
...
> Streaming изначально был исключён, но возвращён в MVP после первой
> попытки прогнать через прокси coding-агент (Qwen Code) — без stream
